...
|---|---|
| [`broadcast_test.py`](broadcast_test.py) | **Instructor alert banner** (`alert`). Four stations (Instructor / Fox / two Hunters): the instructor relays a `msg`, then `stop`, then an `alert` banner, and both hunters confirm reception via the passive `RX B … text=…` line plus a `show` "banner =" cross-check. See `docs/plan-instructor-broadcast.md` and `docs/plan-alert-tone.md`. |
| [`broadcast_display_test.py`](broadcast_display_test.py) | Alert banner **display** check: halts the Fox, force-blanks both Hunter panels, then sends an `alert` and asserts each Hunter **wakes** (`# screen: woke`) and holds the banner lit (no re-blank) — i.e. the banner overrides idle-blank and stays up for its minute. |
| [`alert_tone_test.py`](alert_tone_test.py) | Alert **attention tone + mute override** (`docs/plan-alert-tone.md`). `mute on` both Hunters, send one `alert`, and assert each still sounds the ~1.5 s tone (`# alert: tone`; the Wio's tone is proven via the request/response banner cross-check since its passive log stalls), that mute is **still on** afterward (the override is transient), and that `alert clear` is silent. Snapshots and restores each Hunter's prior mute state. No Fox needed. |
...
| [`msg_text_test.py`](msg_text_test.py) | **Text-frame canned-message mode** (`msgmode text`, field note §7). Fully unattended: flashes the current build to the Fox + Heltec Hunter over serial and the Wio Hunter over its **BLE NUS** (USB-CDC is flaky right after an nrfutil reflash), leaving the Cardputer on old fw as a forward-compat witness. Asserts the clue arrives verbatim with zero `?`, the Hunter renders Morse **locally** (`show` `rxtext` `elems>0`), the burst is deduped, `stop`/`start` gates the text burst, `msgmode` persists across reboot, and the old-fw node ignores `MAGIC_TEXT`. Run with `tools/blevenv/bin/python` (needs pyserial + bleak). |
